...
export function KakaoMapManyMarkerCenter({items, className = ''}: {items: ChildrenCenterList[]; className?: string}) {
    const location = useSelector((state: RootState) => state.location);
    const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24">
    <defs>
    ...
    const handleWebViewMessage = (e: any) => {
        const data = JSON.parse(e.nativeEvent.data);
        if (data.type === 'CENTER_DETAIL' && data.payload?.id) {
            navigation.navigate('centerDetail', {id: data.payload.id});
        }
    };
