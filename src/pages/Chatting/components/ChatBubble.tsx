...
export default function ChatBubble({message, time, isMe = false}: ChatBubbleProps) {
    return (
        <View className={`flex-row items-end mb-2 ${isMe ? 'justify-end' : ''}`}>
            {!isMe && <View className="w-6 h-6 bg-gray-300 rounded-full mr-2" />} {/* 상대일때 메시지 원형 뷰 수정필요*/}
            ...
            {isMe && <View className="w-6 h-6 bg-gray-300 rounded-full ml-2" />} {/* 나일때 메시지 원형 뷰 수정필요*/}
        </View>
    );
}
