...
export function useVolunteerDataKeyword(keyword: string = '') {
    ...
    useEffect(() => {
        const fetchVolunteerData = async () => {
            ...
            try {
                const response = await fetch(
                    `http://openapi.1365.go.kr/openapi/service/rest/VolunteerPartcptnService/getVltrSearchWordList?Keyword=${keyword}&schSido=${location.address?.sidoCd}&numOfRows=10&pageNo=${page}`,
                );
                if (!response.ok) throw new Error('API call failed');
                const xml = await response.text();
                const json = xml2Json(xml);
